...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    # ignore non-used classes
    # ignore = dets['class'] == 0 or \
    #          dets['class'] == 9
    # ignore = dets['class'] == [0,9,10,12,13,14,15,17,18,19,20]
    ignore_arr = [0,9,10,12,13,14,15,17,18,19,20]
    dets_list = dets['class'].tolist()
    ignore = []

    for i in range(len(dets_list)):
        if dets_list[i] in ignore_arr:
            ignore.append(False)
        else:
            ignore.append(True)
