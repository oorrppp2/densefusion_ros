...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        ...
        while 1:
            ...
            if input_line[:5] == 'data/':
                ...
                # syn_index = int(str(input_line[-5]))
                # print(syn_index)
    ...
    def __getitem__(self, index):
        ...
        # img_mat.astype(np.float32)
        # image_mat0 = img_mat[:,:,0].copy()
        # img_mat[:,:,0] = img_mat[:,:,2]
        # img_mat[:,:,2] = image_mat0
        ...
        # pp_img = np.transpose(img_masked, (1, 2, 0))
        # # print(" ** image_masked shape : " + str(pp_img.shape))
        # cv2.imshow(str(class_name[obj[idx]-1]) + "pp_img", pp_img)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()

        # print("Before add normal")
        # print(img_masked)

        if self.list[index][:8] == 'data_syn':
            img_masked = img_masked + np.random.normal(loc=0.0, scale=7.0, size=img_masked.shape)

        # print("After add normal")
        # print(img_masked)

        # p_img = np.transpose(img_masked, (1, 2, 0))
        # print(" ** image_masked shape : " + str(p_img.shape))
        # cv2.imshow(str(class_name[obj[idx]-1]), p_img)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        ...
        # print("dataset model_points shape : " + str(model_points.shape))
        # print("cld[obj[idx]] shape : " + str(self.cld[obj[idx]].shape))
        ...
        # print("before norm img_masked")
        # print(img_masked.astype(np.float32))
        #
        # print("after norm img_masked")
        # print(self.norm(img_masked.astype(np.float32)))
